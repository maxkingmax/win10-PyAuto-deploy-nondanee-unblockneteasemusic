...
def getfile():    ########## 下载nondanee的脚本和node程序
    ...
    if is64Windows():
        nodeurl=nodex64
    if os.path.exists(r".\UnblockNeteaseMusic-master\app.js")==False or os.path.exists(r".\UnblockNeteaseMusic-master\node.exe")==False:
        if os.path.exists(r".\code.zip")==False:
            print("正在下载nondanee的脚本")
...
def netease(): ###获取网易云音乐安装目录
    key = winreg.OpenKeyEx(winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\cloudmusic.exe")
...
def setconfigfile(setport=userport):    ###通过修改配置文件设置网易云的http代理及端口号
    cf_dir = path.expandvars(r'%LOCALAPPDATA%\Netease\CloudMusic\config')
    
    proxylist={'Proxy':{'Type': 'http',
               'http': {'Host': '127.0.0.1', 'Password': '', 'Port': '2333', 'UserName': ''}}}
